...
import { addPlugin, burn, CreatePluginArgsV2, Key, PluginAuthorityPairHelperArgsV2, removePlugin, transfer, update, updateCollection, updateCollectionPlugin, updatePlugin } from '../src';
import { createAsset, createAssetWithCollection, createCollection } from './_setupSdk';
import { assertAsset, assertCollection, createUmi, DEFAULT_ASSET, DEFAULT_COLLECTION } from './_setupRaw';
...
test('it can add and remove all owner and update auth managed party plugins to asset', async (t) => {
  ...
  await Promise.all(plugins.map(async (plugin) => addPlugin(umi, {
      asset: asset.publicKey,
      plugin
    }).sendAndConfirm(umi)))
  ...
  await Promise.all(plugins.map(async (plugin) => removePlugin(umi, {
      asset: asset.publicKey,
      plugin
    }).sendAndConfirm(umi)))
