...
import { ExternalPluginInitInfoArgs, PluginAuthorityPairHelperArgsV2 , createExternalPluginInitInfo, isExternalPluginType, pluginAuthorityPairV2 } from "../plugins";

export type AddPluginArgs = Omit<Parameters<typeof addPluginV1>[1], 'plugin'> & {
  ...
  context: Pick<Context, 'payer' | 'programs' | 'identity'>, {
    ...
    if (isExternalPluginType(plugin)) {
      return addExternalPluginV1(context, {
        ...args,
        initInfo: createExternalPluginInitInfo(plugin as ExternalPluginInitInfoArgs),
      })
    }
