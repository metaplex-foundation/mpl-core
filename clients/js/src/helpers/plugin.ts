...
export function checkPluginAuthorities({ authority, pluginTypes, asset, collection }: CheckPluginAuthoritiesArgs): boolean[] {
  ...
  return pluginTypes.map((type) => {
    ...
    if (plugin) {
      if (hasPluginAddressAuthority(auth, plugin.authority)
