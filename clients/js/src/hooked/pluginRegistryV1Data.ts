...
import { PluginRegistryV1AccountData, PluginRegistryV1AccountDataArgs } from '../generated/types/pluginRegistryV1AccountData';
...
> {
  return {
    ...
    deserialize: (
      ...
      let [pluginType, pluginTypeOffset, isUnknown] = [PluginType.Attributes, offset + 1, true]
      try {
        [pluginType, pluginTypeOffset] = getPluginTypeSerializer().deserialize(buffer, offset);
        isUnknown = false;
      } catch (e) {
        // Do nothing, unknown plugin type
      }
      const [authority, authorityOffset] = getPluginAuthoritySerializer().deserialize(buffer, pluginTypeOffset);
      const [pluginOffset, pluginOffsetOffset] = u64().deserialize(buffer, authorityOffset);
...
> {
  return {
    ...
    deserialize: (
      ...
      const [registry, registryOffset] = array(getRegistryRecordSerializer()).deserialize(buffer, keyOffset)

      // TODO deserialize externalPlugins once they are defined, purposefully ignore them now

      return [{
        key,
        registry: registry.filter((record: RegistryRecordWithUnknown) => !record.isUnknown),
        externalPlugins: [],
      }, registryOffset];
