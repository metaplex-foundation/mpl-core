...
use crate::{
    ...
    instruction::accounts::{
        WriteCollectionExternalPluginDataV1Accounts, WriteExternalPluginDataV1Accounts,
    },
    plugins::{
        fetch_wrapped_external_plugin, DataStore, ExternalPlugin, ExternalPluginKey, LifecycleHook,
        PluginHeaderV1, PluginRegistryV1,
    },
...
pub(crate) fn write_external_plugin_data<'a>(
    ...
    args: WriteExternalPluginDataV1Args,
) -> ProgramResult {
    let ctx = WriteExternalPluginDataV1Accounts::context(accounts)?;
...
pub(crate) fn write_collection_external_plugin_data<'a>(
    accounts: &'a [AccountInfo<'a>],
    args: WriteCollectionExternalPluginDataV1Args,
) -> ProgramResult {
    let ctx = WriteCollectionExternalPluginDataV1Accounts::context(accounts)?;
...
fn process_write_external_plugin_data<'a, T: DataBlob + SolanaAccount>(
    ...
    let (record, plugin) = fetch_wrapped_external_plugin::<T>(account, None, key)?;
    match plugin {
        ExternalPlugin::LifecycleHook(LifecycleHook {
            data_authority: Some(data_authority),
            ..
        })
        | ExternalPlugin::DataStore(DataStore { data_authority, .. }) => {
            if !authorities.contains(&data_authority) {
                return Err(MplCoreError::InvalidAuthority.into());
            }
        }
        ExternalPlugin::Oracle(_) => todo!(),
