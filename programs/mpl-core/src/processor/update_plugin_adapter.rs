...
use crate::{
    ...
    instruction::accounts::{
        UpdateCollectionPluginAdapterV1Accounts, UpdatePluginAdapterV1Accounts,
    },
    plugins::{
        fetch_wrapped_plugin_adapter, find_plugin_adapter, Plugin, PluginAdapter, PluginAdapterKey,
        PluginAdapterUpdateInfo, PluginHeaderV1, PluginRegistryV1, PluginType,
    },
...
pub(crate) struct UpdatePluginAdapterV1Args {
    /// Plugin adapter key.
    pub key: PluginAdapterKey,
    /// Plugin info to update.
    pub update_info: PluginAdapterUpdateInfo,
}

pub(crate) fn update_plugin_adapter<'a>(
    accounts: &'a [AccountInfo<'a>],
    args: UpdatePluginAdapterV1Args,
) -> ProgramResult {
    // Accounts.
    let ctx = UpdatePluginAdapterV1Accounts::context(accounts)?;
    ...
    let (_, plugin) = fetch_wrapped_plugin_adapter::<AssetV1>(ctx.accounts.asset, None, &args.key)?;
    ...
    let (mut asset, plugin_header, plugin_registry) = validate_asset_permissions(
        ...
        None,
        Some(&plugin),
        AssetV1::check_update_plugin_adapter,
        CollectionV1::check_update_plugin_adapter,
        PluginType::check_update_plugin_adapter,
        AssetV1::validate_update_plugin_adapter,
        CollectionV1::validate_update_plugin_adapter,
        Plugin::validate_update_plugin_adapter,
        None,
        None,
    )?;
...
#[repr(C)]
#[derive(BorshSerialize, BorshDeserialize, PartialEq, Eq, Debug, Clone)]
pub(crate) struct UpdateCollectionPluginAdapterV1Args {
    /// Plugin adapter key.
    pub key: PluginAdapterKey,
    /// Plugin info to update.
    pub update_info: PluginAdapterUpdateInfo,
...
pub(crate) fn update_collection_plugin_adapter<'a>(
    accounts: &'a [AccountInfo<'a>],
    args: UpdateCollectionPluginAdapterV1Args,
) -> ProgramResult {
    // Accounts.
    let ctx = UpdateCollectionPluginAdapterV1Accounts::context(accounts)?;
    ...
    let (_, plugin) =
        fetch_wrapped_plugin_adapter::<CollectionV1>(ctx.accounts.collection, None, &args.key)?;
...
fn process_update_plugin_adapter<'a, T: DataBlob + SolanaAccount>(
    core: T,
    plugin: PluginAdapter,
    key: PluginAdapterKey,
    update_info: PluginAdapterUpdateInfo,
    ...
    let (_, record) = find_plugin_adapter(&plugin_registry_clone, &key, account)?;
